package com.poomoo.edao.crashhandler;
...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	private boolean handleException(final Throwable ex) {
		...
		new Thread() {
			...
			public void run() {
				...
				Looper.prepare();
				Utity.showToast(mContext, "我们的程序碰到了点小问题,请重新启动^_^");
				Looper.loop();
			}
		}.start();
	...
	private String saveCrashInfo2File(Throwable ex) {
		...
		try {
			...
			File dir = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/eDaoClient/");
			if (!dir.exists()) {
				dir.mkdirs();
			}
			cmd = "chmod 777 " + Environment.getExternalStorageDirectory().getAbsolutePath() + "/eDaoClient/";
			Runtime.getRuntime().exec(cmd);
			FileOutputStream fos = new FileOutputStream(
					Environment.getExternalStorageDirectory().getAbsolutePath() + "/eDaoClient/" + fileName);
